...
class BotState:
    ...
    dry_run: bool = DRY_RUN  # True = paper trading; False = live


    position: Optional[Position] = None
    sim_balance_usdt: float = SIMULATED_BALANCE_USDT
    ...
    event_log: deque = field(default_factory=lambda: deque(maxlen=MAX_EVENT_LOG))

    order_mode: str = "market"  # "market" or "limit"
    open_order: Optional[OpenOrder] = None

    # Live-tunable params
    buy_threshold_pct: float = BUY_THRESHOLD_PCT_DEFAULT
    sell_threshold_pct: float = SELL_THRESHOLD_PCT_DEFAULT
    manual_buy_pct: float = MANUAL_BUY_PCT_DEFAULT  # 0..1 fraction of USDT
    auto_buy_pct: float = AUTO_BUY_PCT_DEFAULT      # 0..1 fraction of USDT


    manual_sell_pct: float = MANUAL_SELL_PCT_DEFAULT  # 0..1 fraction of position
    ...
    data_driven: bool = DATA_DRIVEN_DEFAULT
    data_timeframe: str = DATA_TIMEFRAME_DEFAULT
    data_lookback: int = DATA_LOOKBACK_CANDLES_DEFAULT
    data_refresh_sec: int = DATA_REFRESH_SEC_DEFAULT

    avg_high: float = 0.0
    avg_low: float = 0.0
    avg_range_pct: float = 0.0
    data_trend: str = "?"
    effective_buy_threshold_pct: float = BUY_THRESHOLD_PCT_DEFAULT
    effective_sell_threshold_pct: float = SELL_THRESHOLD_PCT_DEFAULT
    _last_data_refresh_ts: float = 0.0


    # --- Adaptive profiles (multi-horizon) ---
    adaptive_profile: str = ADAPTIVE_PROFILE_DEFAULT
    adaptive_timeframes: List[str] = field(default_factory=lambda: list(ADAPTIVE_PRESETS[ADAPTIVE_PROFILE_DEFAULT]["timeframes"]))
    adaptive_weights: List[float] = field(default_factory=list)  # used in Custom (optional)
    adaptive_k_buy: float = float(ADAPTIVE_PRESETS[ADAPTIVE_PROFILE_DEFAULT]["k_buy"])
    adaptive_k_sell: float = float(ADAPTIVE_PRESETS[ADAPTIVE_PROFILE_DEFAULT]["k_sell"])
    adaptive_alpha: float = float(ADAPTIVE_PRESETS[ADAPTIVE_PROFILE_DEFAULT]["alpha"])
...
def save_settings(state: "BotState") -> None:
    ...
    try:
        ...
        with state.lock:
            payload = {
                "symbol": SYMBOL,
                "dry_run": bool(getattr(state, "dry_run", DRY_RUN)),
                ...
                "manual_sell_pct": float(getattr(state, "manual_sell_pct", MANUAL_SELL_PCT_DEFAULT)),
                ...
                "data_timeframe": str(getattr(state, "data_timeframe", DATA_TIMEFRAME_DEFAULT)),
                "data_lookback": int(getattr(state, "data_lookback", DATA_LOOKBACK_CANDLES_DEFAULT)),
                "data_refresh_sec": int(getattr(state, "data_refresh_sec", DATA_REFRESH_SEC_DEFAULT)),
                "adaptive_profile": str(getattr(state, "adaptive_profile", ADAPTIVE_PROFILE_DEFAULT)),
                "adaptive_timeframes": list(getattr(state, "adaptive_timeframes", [])),
                "adaptive_weights": list(getattr(state, "adaptive_weights", [])),
                "adaptive_k_buy": float(getattr(state, "adaptive_k_buy", ADAPTIVE_PRESETS[ADAPTIVE_PROFILE_DEFAULT]["k_buy"])),
                "adaptive_k_sell": float(getattr(state, "adaptive_k_sell", ADAPTIVE_PRESETS[ADAPTIVE_PROFILE_DEFAULT]["k_sell"])),
                "adaptive_alpha": float(getattr(state, "adaptive_alpha", ADAPTIVE_PRESETS[ADAPTIVE_PROFILE_DEFAULT]["alpha"])),
                ...
                "data_trend": str(getattr(state, "data_trend", "?")),
                ...
                "DAILY_MAX_PROFIT_USDT": DAILY_MAX_PROFIT_USDT,
                "ANCHOR_MAX_AGE_SEC": ANCHOR_MAX_AGE_SEC,
                "ANCHOR_DRIFT_REARM_PCT": ANCHOR_DRIFT_REARM_PCT,
                "POLL_INTERVAL_SECONDS": POLL_INTERVAL_SECONDS,
                "MIN_NOTIONAL_USDT": MIN_NOTIONAL_USDT,
                "MAX_STOPS_PER_HOUR": MAX_STOPS_PER_HOUR,
                "CIRCUIT_STOP_COOLDOWN_SEC": CIRCUIT_STOP_COOLDOWN_SEC,
            }
...
def fetch_ohlcv_df(exchange: ccxt.Exchange) -> Optional[pd.DataFrame]:
    try:
    ...
    df["datetime"] = pd.to_datetime(df["timestamp"], unit="ms", utc=True).dt.tz_convert(None)  # naive for mpl
    return df
